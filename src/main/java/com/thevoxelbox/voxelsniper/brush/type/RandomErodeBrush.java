...
public class RandomErodeBrush extends AbstractBrush {
    ...
    private void setSnap(final int brushSize, final Block targetBlock) {
    ...
    private void randomErosion(final Snipe snipe) {
        Undo undo = new Undo();
        ...
        for (final BlockWrapper[][] firstSnapSlice : this.firstSnap) {
            for (final BlockWrapper[] firstSnapString : firstSnapSlice) {
    ...
    private void randomFilling(final Snipe snipe) {
        Undo undo = new Undo();
        if (this.fillFace >= 0 && this.fillFace <= 6) {
    ...
    private static final class BlockWrapper {
        ...
        private final Material nativeType;

...
public abstract class AbstractStampBrush extends AbstractBrush {
    ...
    protected boolean falling(final Material material) {
        return MaterialSets.FALLING.contains(material);
    }

    protected boolean fallsOff(final Material material) {
    ...
    protected void setBlock(final StampBrushBlockWrapper blockWrapper) {
    ...
    protected void setBlockFill(final StampBrushBlockWrapper blockWrapper) {
        Block targetBlock = getTargetBlock();
        Block block = clampY(targetBlock.getX() + blockWrapper.getX(), targetBlock.getY() + blockWrapper.getY(), targetBlock.getZ() + blockWrapper.getZ());
    ...
    protected void stamp(final Snipe snipe) {
        this.undo = new Undo();
        if (this.sorted) {
            for (final StampBrushBlockWrapper block : this.solid) {
                setBlock(block);
            }
            for (final StampBrushBlockWrapper block : this.drop) {
                setBlock(block);
            }
            for (final StampBrushBlockWrapper block : this.fall) {
                setBlock(block);
            }
        } else {
            this.fall.clear();
            this.drop.clear();
            this.solid.clear();
            for (final StampBrushBlockWrapper block : this.clone) {
    ...
    protected void stampFill(final Snipe snipe) {
        this.undo = new Undo();
        if (this.sorted) {
            for (final StampBrushBlockWrapper block : this.solid) {
    ...
    protected void stampNoAir(final Snipe snipe) {
        this.undo = new Undo();
        if (this.sorted) {
            for (final StampBrushBlockWrapper block : this.solid) {
                this.setBlock(block);
    ...
    public void setStamp(final StampType stamp) {
        this.stamp = stamp;
    }
}

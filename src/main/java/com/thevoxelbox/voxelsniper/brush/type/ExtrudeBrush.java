...
public class ExtrudeBrush extends AbstractBrush {
    ...
    private void extrudeUpOrDown(final Snipe snipe, final boolean isUp) {
    ...
    private void extrudeNorthOrSouth(final Snipe snipe, final boolean isSouth) {
        ToolkitProperties toolkitProperties = snipe.getToolkitProperties();
        int brushSize = toolkitProperties.getBrushSize();
        double brushSizeSquared = Math.pow(brushSize + this.trueCircle, 2);
        Undo undo = new Undo();
    ...
    private void extrudeEastOrWest(final Snipe snipe, final boolean isEast) {
        ToolkitProperties toolkitProperties = snipe.getToolkitProperties();
        int brushSize = toolkitProperties.getBrushSize();
        double brushSizeSquared = Math.pow(brushSize + this.trueCircle, 2);
        Undo undo = new Undo();
    ...
    private void perform(final Block block1, final Block block2, final ToolkitProperties toolkitProperties, final Undo undo) {
    ...
    private void selectExtrudeMethod(final Snipe snipe, @Nullable final BlockFace blockFace, final boolean towardsUser) {
        ToolkitProperties toolkitProperties = snipe.getToolkitProperties();

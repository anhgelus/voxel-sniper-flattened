...
/**
 * FOR ANY BRUSH THAT USES A SPLINE, EXTEND THAT BRUSH FROM THIS BRUSH!!! That way, the spline
 * calculations are already there. Also, the UI for
...
public class SplineBrush extends AbstractPerformerBrush {
    ...
    private void addToSet(final Snipe snipe, final boolean ep, final Block targetBlock) {
        SnipeMessenger messenger = snipe.createMessenger();
        if (ep) {
    ...
    private void removeFromSet(final Snipe snipe, final boolean ep, final Block targetBlock) {
        SnipeMessenger messenger = snipe.createMessenger();
        if (ep) {
    ...
    private boolean spline(final Point start, final Point end, final Point c1, final Point c2, final Snipe snipe) {
        SnipeMessenger messenger = snipe.createMessenger();
        this.spline.clear();
        try {
    ...
    private void render(final Snipe snipe) {
    ...
    private void clear(final Snipe snipe) {
        this.spline.clear();
        this.ctrlPts.clear();
        this.endPts.clear();
        SnipeMessenger messenger = snipe.createMessenger();
    ...
    private static final class Point {
        ...
        private Point(final Block block) {
            this.x = block.getX();
            this.y = block.getY();
            this.z = block.getZ();
        }

        private Point(final int x, final int y, final int z) {
            this.x = x;
            this.y = y;
            this.z = z;
        }
        ...
        public Point multiply(final int scalar) {

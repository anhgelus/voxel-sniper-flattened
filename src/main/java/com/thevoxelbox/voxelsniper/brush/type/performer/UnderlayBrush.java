...
public class UnderlayBrush extends AbstractPerformerBrush {
    ...
    @Override
    public final void handleCommand(final String[] parameters, final Snipe snipe) {
    ...
    private void underlay(final Snipe snipe) {
    ...
    private void underlay2(final Snipe snipe) {
        ToolkitProperties toolkitProperties = snipe.getToolkitProperties();
        int brushSize = toolkitProperties.getBrushSize();
        int[][] memory = new int[brushSize * 2 + 1][brushSize * 2 + 1];
        double brushSizeSquared = Math.pow(brushSize + 0.5, 2);

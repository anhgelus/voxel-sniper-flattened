...
public class PullBrush extends AbstractBrush {
    ...
    public void handleArrowAction(final Snipe snipe) {
        ...
        if (this.voxelHeight > 0) {
            for (final PullBrushBlockWrapper block : this.surface) {
                setBlock(block);
            }
    ...
    private void getSurface(final ToolkitProperties toolkitProperties) {
        this.surface.clear();
        int brushSize = toolkitProperties.getBrushSize();
    ...
    private boolean isEmpty(final int x, final int y, final int i) {
        Material type = getBlockType(x, y, i);
        return Materials.isEmpty(type);
    }

    private void setBlock(final PullBrushBlockWrapper block) {
    ...
    private void setBlockDown(final PullBrushBlockWrapper block) {
        Block currentBlock = this.clampY(block.getX(), block.getY() + (int) (this.voxelHeight * block.getStr()), block.getZ());
        currentBlock.setBlockData(block.getBlockData());
        for (int y = block.getY(); y > currentBlock.getY(); y--) {
    ...
    private static final class PullBrushBlockWrapper {
        ...
        private PullBrushBlockWrapper(final Block block, final double str) {

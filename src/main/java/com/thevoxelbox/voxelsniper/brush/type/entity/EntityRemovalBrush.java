...
public class EntityRemovalBrush extends AbstractBrush {
    ...
    public void handleCommand(final String[] parameters, final Snipe snipe) {
        ...
        for (final String currentParam : parameters) {
            ...
            if (currentParam.equalsIgnoreCase("list-exemptions") || currentParam.equalsIgnoreCase("lex")) {
                for (final String exemption : this.exemptions) {
    ...
    private void radialRemoval(final Snipe snipe) {
        SnipeMessenger messenger = snipe.createMessenger();
    ...
    private boolean isClassInExemptionList(final Class<? extends Entity> entityClass) {

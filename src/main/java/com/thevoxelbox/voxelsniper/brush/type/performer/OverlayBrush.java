...
public class OverlayBrush extends AbstractPerformerBrush {
    ...
    private void overlay(final Snipe snipe) {
    ...
    private boolean isIgnoredBlock(final Material material) {
        return material == Material.WATER || material.isTransparent() || material == Material.CACTUS;
    }

    private boolean isOverrideableMaterial(final Material material) {
    ...
    private void overlayTwo(final Snipe snipe) {
        ToolkitProperties toolkitProperties = snipe.getToolkitProperties();
        int brushSize = toolkitProperties.getBrushSize();
        double brushSizeSquared = Math.pow(brushSize + 0.5, 2);
